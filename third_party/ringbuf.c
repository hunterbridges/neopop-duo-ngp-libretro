...
const uint8_t *
ringbuf_buf(const struct ringbuf *rb)
{
    return rb->buf;
}
...
ringbuf_findchr(const struct ringbuf *rb, int c, size_t offset)
{
    ...
    assert(bufend > start);
    size_t n = MIN((size_t)(bufend - start), (size_t)(bytes_used - offset));
...
ringbuf_memset(ringbuf_t dst, int c, size_t len)
{
    ...
    while (nwritten != count) {
        ...
        size_t n = MIN((size_t)(bufend - dst->head), (size_t)(count - nwritten));
        memset(dst->head, c, n);
        dst->head += n;
        nwritten += n;
...
ringbuf_memcpy_into(ringbuf_t dst, const void *src, size_t count)
{
    ...
    while (nread != count) {
        /* don't copy beyond the end of the buffer */
        assert(bufend > dst->head);
        size_t n = MIN((size_t)(bufend - dst->head), (size_t)(count - nread));
...
ringbuf_memcpy_from(void *dst, ringbuf_t src, size_t count)
{
    ...
    while (nwritten != count) {
        ...
        size_t n = MIN((size_t)(bufend - src->tail), (size_t)(count - nwritten));
...
ringbuf_copy(ringbuf_t dst, ringbuf_t src, size_t count)
{
    ...
    while (ncopied != count) {
        ...
        size_t nsrc = MIN((size_t)(src_bufend - src->tail), (size_t)(count - ncopied));
        assert(dst_bufend > dst->head);
        size_t n = MIN((size_t)(dst_bufend - dst->head), (size_t)(nsrc));
